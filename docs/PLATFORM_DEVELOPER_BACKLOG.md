...
|:---|:---|:---|:---|:---:|
| **PF-01** | Application Function Runtime | **Частично** | Расширить script engine (§4.1) | P0 |
| **PF-02** | Application Data Layer | **Частично** | Schema isolation, seed API (§4.2) | P0 |
...
Выборка списка строк для BFF-таблиц.

```json
{ "type": "selectMany", "var": "orders", "sql": "SELECT ...", "params": ["${input.shiftId}"] }
```

- Результат: `List<Map>` в vars.
- Шаг `return` должен уметь отдавать массив в поле output (табличный `result` на wire).

**Покрывает:** `terminal_dispatchWorkbench_listActiveOrders`, `terminal_qualityGate_listTankStatus`, `terminal_tankMonitoring_*`.

#### PF-01b — `invoke_function`

Вызов другой deploy-функции из script (композиция без Java).

```json
{
  "type": "invoke_function",
  "var": "gateResult",
  "objectPath": "root.platform.terminal.config.qualityGate",
  "functionName": "terminal_qualityGate_assertTankApproved",
  "input": { "tankPath": "${order.tank_path}" }
}
```

- Propagate `error_code != OK` → выход текущей функции.
- Лимит глубины вызовов (например 8) против циклов.

**Покрывает:** `startFilling` → quality gate; ERP post fact из close.
...
Один invoke = одна JDBC transaction (rollback при необ необработанном exception).

**Acceptance PF-01:**

- [ ] 16 функций `terminal_*` (§6) реализуемы **только** JSON script + SQL.
- [ ] Deploy идемпотентен; invalid script → 400 на deploy.
- [ ] RBAC как у `functions/invoke`.
- [ ] Документация: `docs/APPLICATIONS.md` § Script steps (полный список).
...
- `tablePrefix` или dedicated schema `app_{appId}` применяется автоматически **или** documented convention + validation.
- Запрет DDL в migrate без prefix/schema app (guard).
...
- [ ] Повтор migrate/seed — no duplicate rows.
- [ ] Две app schema не пересекаются.
